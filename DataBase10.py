...
class DataBase10():
    ...
    def getLast(self):
        ...
        tmpX = np.copy(xn[0, 10:])   # Xk+1
        tmpU = np.copy(xn[0, 8:10])  # Uk  we assume the next u will be close to the previous
    ...
    def getAll(self):
        ...
        for i in range(0, self.size):
            tmpOut = self.Q.pop()
            input[i, :] = np.copy(tmpOut[0, :10])
            target[i, :] = np.copy(tmpOut[0, 10:])
        for i in range(0, self.size):
